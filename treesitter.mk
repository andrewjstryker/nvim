...
#
# Design:
#   provision-parsers runs headless under the installed config and executes
...
#   - The config is installed. rocks-sync is the normal prerequisite that makes
#     nvim-treesitter available before parser provisioning begins.
#
#-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=#
...
#
# Needed because nvim-treesitter's installer treats a .so already sitting in the
...
# holds nothing but compiled output; the next sync rebuilds it.
#------------------------------------------------------------------------------#
...
clean-parsers:
	...
	@printf "\033[1;32mParser tree removed. Run \`make sync\` to reprovision.\033[0m\n"
