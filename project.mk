...
#   update-config-env is a phony reconciliation action, but ${config_env} is a
#   real file replaced only when its content actually changes (via cmp -s).
#   This bridges environment changes into Make's mtime graph:
#
...
#   propagate.
#
#   Do not mark the real file .PHONY: that verdict would propagate and rebuild
#   every dependent regardless of its mtime. Stage runs the separate phony
#   action before realizing outputs; dependents compare against the real file.
#
define reconcile_config_env
	@mkdir -p "$(dir ${config_env})"
	@set -eu; \
	  tmp="$$(mktemp "$(dir ${config_env})/.config_env.m4.XXXXXX")"; \
	  { \
	  ...
	  if [ ! -f "${config_env}" ] || ! cmp -s "$$tmp" "${config_env}"; then \
	    mv "$$tmp" "${config_env}"; \
	  else \
	    rm -f "$$tmp"; \
	  fi
...
.PHONY: update-config-env
update-config-env:
	$(reconcile_config_env)

${config_env}: update-config-env ;

stage: update-config-env
